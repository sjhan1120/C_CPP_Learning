...
int main()
{
    struct Score    Student1;                           //(2) 구조체(설계도)를 바탕으로 메모리에 정적으로 Student1  객체 생성
    struct Score    *pStudent2=new struct Score;        //(3) 구조체(설계도)를 바탕으로 메모리에 동적으로 pStudent2 객체 생성
